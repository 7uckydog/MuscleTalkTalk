...
//	}
	...
	public int insertRoutine(Connection conn, RoutineVo vo, ArrayList<RoutineExerciseVo> rouExerVoList) {
		...
		String sql = "insert all"
				...
				+ "	            values ((select nvl(max(ROUTINE_NO),0)+1 from tb_routine), "+memberNo+", ?, 'f', ?, ?, 'default')"
				+ "	    ";
		...
		try {
			...
			pstmt.setString(1, vo.getRoutineName());
			pstmt.setString(2, vo.getRoutineTarget()); //목표부위 char형
			pstmt.setString(3, vo.getRoutineContent());
		
			
			for(int i = 0; i < rouExerVoList.size(); i++) {
				pstmt.setInt(4+i, rouExerVoList.get(i).getExerciseNo());      // 운동번호 notnull
				pstmt.setInt(5+i, rouExerVoList.get(i).getRoutineExerciseDay());  // 운동요일 notnull
				pstmt.setInt(6+i, rouExerVoList.get(i).getRoutineWeek());  // 루틴수행주차
				pstmt.setInt(7+i, rouExerVoList.get(i).getRoutineDay());   //  루틴수행n번째일
				pstmt.setInt(8+i, rouExerVoList.get(i).getRoutineExerciseSet());   // 루틴운동 세트수
				pstmt.setInt(9+i, rouExerVoList.get(i).getRoutineExerciseRepeat());  // 루틴운동 반복횟수
				pstmt.setInt(10+i, rouExerVoList.get(i).getRoutineExerciseWeight());  // 루틴무게
				pstmt.setInt(11+i, rouExerVoList.get(i).getRoutineExerciseTime());  // 루틴운동시간
				pstmt.setInt(12+i, rouExerVoList.get(i).getRoutineExerciseDistance());  // 루틴운동 거리
				pstmt.setInt(13+i, rouExerVoList.get(i).getRoutineExerciseSequence());  // n번째운동
			}
