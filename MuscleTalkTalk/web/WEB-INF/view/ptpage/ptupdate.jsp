	...
	<section id="pt_insert_page_section">
		...
		<div id="pt_write_content">
			<form name="pt_time_form" action="ptinsert.do" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="pt_update_page_ptNo" id="pt_update_page_ptNo">
			</form>
		</div>
	</section>
...
/* 		$("#pt_name").val("프로그램명을 입력하세요");
		...
		$(function() {
			...
			var nextDay = new Date('${pVo.ptStartDate}');
		...
		$(function() {
			$("#pt_name").val('${pVo.ptName}');
			$("#pt_name").css('color', 'black');
			var ptCategoryInt = ${pVo.ptCategory};
			$("#pt_category").val(Number(ptCategoryInt));
			var ptCategory;
			switch (ptCategoryInt) {
			  case 1:
				  ptCategory = '웨이트';
			    break;
			  case 2:
			...
			$("#input_imgbox2").css(
					{
						"background-image" : "url(${pVo.ptFilePathList[1]})",
						"outline" : "none",
						"background-size" : "contain",
						"background-repeat" : "no-repeat",
						"background-position" : "center"
			...
			$("#input_imgbox3").css(
					{
						"background-image" : "url(${pVo.ptFilePathList[2]})",
						"outline" : "none",
						"background-size" : "contain",
						"background-repeat" : "no-repeat",
						"background-position" : "center"
			...
			$(".time_info_div").eq(0).append("");
			$(".time_info_div").eq(0).children().eq(0).text(ptTimeInfoArr[0]);
			$(".time_info_div").eq(0).children().eq(1).text(ptTimeInfoArr[1]);
			$(".time_info_div").eq(0).children().eq(2).text(ptTimeInfoArr[2]);
			for(var i = 3; i < ptTimeInfoArr.length; i = i + 3) {
				var dayTemp = (i/3) - 1;
				$(".time_info_div").eq(dayTemp).after('<div class="time_info_div"></div>');
				for (var j = 0; j < 3; j++) {
					$(".time_info_div").eq(dayTemp + 1).append('<p class="time_info_p"></p>');
				}
				$(".time_info_div").eq(dayTemp + 1).append("");
				$(".time_info_div").eq(dayTemp + 1).children().eq(0).text(ptTimeInfoArr[i+0]);
